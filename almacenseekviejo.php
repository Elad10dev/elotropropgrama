...
if ($_POST['go'] == 'datatable') {
    ...
    $query2 = "SELECT a.Cashea_AliadoName,a.Cashea_APIKEY,a.Cashea_Precio,a.codigocontable,a.IdAtt,a.FormaOC,a.FormPresu,a.FormPedid,a.FormaFac,a.FormaBol,a.FormaGuia,a.FormaNote,a.FormaMovi,a.ImpFac,a.impBoleta,a.ImpGuia,a.ImpNotaEnt,a.ImpMovInventario, a.IdAlm as IdAlm,
    ...
    while ($row = mysqli_fetch_array($query)) {
        ...
        $subdata[] = "<span class='' id='nom" . $n . "'>" . $row['Nombre'] . "</span>" . '<br>
            ...
            <label style="display:none" class="text-center" id="Cashea_AliadoName' . $n . '">' . trim($row['Cashea_AliadoName']) . '</label>
            <label style="display:none" class="text-center" id="Cashea_APIKEY' . $n . '">' . trim($row['Cashea_APIKEY']) . '</label>
            <label style="display:none" class="text-center" id="Cashea_Precio' . $n . '">' . trim($row['Cashea_Precio']) . '</label>
            <label style="display:none" class="text-center" id="FormOC' . $n . '">' . trim($row['FormaOC']) . '</label>';
        $subdata[] = $row['Nombre'];
        $tipo = '';
...
if ($_POST["tabla"] == "almacen") {
    ...
    if (!isset($_POST["borrar"])) {
        ...
        if ($_POST["ModalIdAlm"] == "") {
            $sql = "insert into PosUpAlmacen (FormaOC,FormPresu,IdAlm,Nombre,Tipo,IdCompany,IdUbi,ImpFac,impBoleta,ImpGuia,ImpNotaEnt,ImpMovInventario,FormaFac,FormaBol,FormaGuia,FormaNote,FormaMovi,FormPedid,IdAtt,codigocontable,Cashea_AliadoName,Cashea_APIKEY,Cashea_Precio) (select '" . trim($FormOC) . "','" . trim($FormPresu) . "', COALESCE(max(IdAlm),0)+1 as IdAlm,'" . trim($ModalNombre) . "'," . trim($ModalTipo) . "," . $_POST["companyUser"] . "," . trim($ModalIdUbi) . "," . trim($ImpFac) . "," . trim($impBoleta) . "," . trim($ImpGuia) . "," . trim($ImpNotaEnt) . "," . trim($ImpMovInventario) . ",'" . trim($FormaFac) . "','" . trim($FormaBol) . "','" . trim($FormaGuia) . "','" . trim($FormaNote) . "','" . trim($FormaMovi) . "','" . trim($FormPedid) . "','" . $ModalAtencion . "','" . $ModalCodigoContable . "','" . $_POST["CasheAliado"] . "','" . $_POST["CasheApikey"] . "','" . $_POST["CasheaPrecio"] . "' from PosUpAlmacen where IdCompany = " . $_POST["companyUser"] . ")";
        } else {
            $sql = "update PosUpAlmacen set Cashea_AliadoName = '" . $_POST["CasheAliado"] . "',Cashea_APIKEY = '" . $_POST["CasheApikey"] . "' ,Cashea_Precio = '" . $_POST["CasheaPrecio"] . "',codigocontable='" . trim($ModalCodigoContable) . "',IdAtt='" . trim($ModalAtencion) . "',FormaOC='" . trim($FormOC) . "',FormPresu='" . trim($FormPresu) . "',FormPedid='" . trim($FormPedid) . "',FormaFac='" . trim($FormaFac) . "',FormaBol='" . trim($FormaBol) . "',FormaGuia='" . trim($FormaGuia) . "',FormaNote='" . trim($FormaNote) . "',FormaMovi='" . trim($FormaMovi) . "',ImpFac='" . trim($ImpFac) . "',impBoleta='" . trim($impBoleta) . "',ImpGuia='" . trim($ImpGuia) . "',ImpNotaEnt='" . trim($ImpNotaEnt) . "',ImpMovInventario='" . trim($ImpMovInventario) . "',Nombre='" . trim($ModalNombre) . "',Tipo='" . trim($ModalTipo) . "',IdUbi= " . trim($ModalIdUbi) . " where IdAlm=" . trim($_POST["ModalIdAlm"]) . " and IdCompany=" . trim($_POST["companyUser"]);
        }
