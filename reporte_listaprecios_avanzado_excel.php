...
// INICIAMOS LA CAPTURA DEL CÓDIGO HTML
ob_start();
...
if ($n == 0) {
    echo "<script>
        ...
        // Si está en la misma pestaña (Excel), regresa atrás
        window.history.back();
        // Si está en una pestaña nueva (PDF), la cierra
...
?>
                <tr>
                    ...
                    <?php if (($_POST["costozzzv"] ?? '') == "on") { echo '<td align="right">'.(trim($row['costo']) == "" ? "-" : number_format($row['costo'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda).'</td>'; } ?>
                    <?php if (($_POST["precioz"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto']) == "" ? "-" : number_format($row['PrecioNeto'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda).'</td>'; } ?>
                    <?php if (($_POST["precio2z"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto2']) == "" ? "-" : number_format($row['PrecioNeto2'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda . ($row['CantP1']>1 ? ' / ' . number_format(trim($row['PrecioNeto2']) / $row['CantP1'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) : '')).'</td>'; } ?>
                    <?php if (($_POST["precio3z"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto3']) == "" ? "-" : number_format($row['PrecioNeto3'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda . ($row['CantP2']>1 ? ' / ' . number_format(trim($row['PrecioNeto3']) / $row['CantP2'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) : '')).'</td>'; } ?>
                    <?php if (($_POST["precio4z"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto4']) == "" ? "-" : number_format($row['PrecioNeto4'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda . ($row['CantP4']>1 ? ' / ' . number_format(trim($row['PrecioNeto4']) / $row['CantP4'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) : '')).'</td>'; } ?>
                    <?php if (($_POST["precio5z"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto5']) == "" ? "-" : number_format($row['PrecioNeto5'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda . ($row['CantP5']>1 ? ' / ' . number_format(trim($row['PrecioNeto5']) / $row['CantP5'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) : '')).'</td>'; } ?>
                    <?php if (($_POST["precio6z"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto6']) == "" ? "-" : number_format($row['PrecioNeto6'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda . ($row['CantP6']>1 ? ' / ' . number_format(trim($row['PrecioNeto6']) / $row['CantP6'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) : '')).'</td>'; } ?>
                    <?php if (($_POST["precio7z"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto7']) == "" ? "-" : number_format($row['PrecioNeto7'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda . ($row['CantP7']>1 ? ' / ' . number_format(trim($row['PrecioNeto7']) / $row['CantP7'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) : '')).'</td>'; } ?>
                    <?php if (($_POST["precio8z"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto8']) == "" ? "-" : number_format($row['PrecioNeto8'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda . ($row['CantP8']>1 ? ' / ' . number_format(trim($row['PrecioNeto8']) / $row['CantP8'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) : '')).'</td>'; } ?>
                    <?php if (($_POST["precio9z"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto9']) == "" ? "-" : number_format($row['PrecioNeto9'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda . ($row['CantP9']>1 ? ' / ' . number_format(trim($row['PrecioNeto9']) / $row['CantP9'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) : '')).'</td>'; } ?>
                    <?php if (($_POST["precio10z"] ?? '') == "on") { echo '<td align="right">'.(trim($row['PrecioNeto10']) == "" ? "-" : number_format($row['PrecioNeto10'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) . " " . $moneda . ($row['CantP10']>1 ? ' / ' . number_format(trim($row['PrecioNeto10']) / $row['CantP10'], $_POST["CD"], $_POST["SimDec"], $_POST["SimMil"]) : '')).'</td>'; } ?>
                    <?php if (($_POST["cimp"] ?? '') == "on") { echo '<td align="right">'.(trim($row['ValorImpuesto']) == "" ? "-" : trim($row['ValorImpuesto'])).'</td>'; } ?>
                </tr>
<?php
            }
...
$nombreArchivo = "Lista_Precios_Avanzada_" . date('Ymd_His') . ".xls";
...
<body>
    ...
    <table width="100%">
        <tr>
            <td colspan="15" align="center" style="font-size: 16pt; font-weight: bold; color: #1a73e8;">
                LISTA DE PRECIOS AVANZADA
            </td>
        </tr>
        <tr>
            <td colspan="15" align="center" style="font-size: 12pt; font-weight: bold;">
                <?php echo mb_strtoupper($_POST["NameCompany"] ?? ''); ?>
            </td>
        </tr>
        <tr>
            <td colspan="15" align="center">
                <b>Expresado en:</b> <?php echo $moneda; ?> (Tasa: <?php echo number_format($tasa, 2); ?>) | 
