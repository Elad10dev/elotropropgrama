<?php
/**
 * exportar_retencion_txt.php
 * Genera el archivo TXT de ISLR aplicando la misma lógica blindada del XML.
 * (Búsqueda del Código SENIAT en BD + Extracción de montos con decimales en coma).
 */
...
header('Content-Disposition: attachment; filename="Retenciones_ISLR_'.$periodo.'.txt"');
...
while ($row = $result->fetch_assoc()) {
    ...
    // IMPRESIÓN DEL NODO TXT (Separado por tabulaciones y con coma decimal)
    // Formato SENIAT: RIF Agente | Periodo | RIF Retenido | Factura | Control | Fecha | Concepto | Monto Base | % Retencion
    $linea = implode("\t", [
        $rifAgente,
        $periodo,
        $rifRetenido,
        ...
        $fechaOperacion,
        $codigoConcepto,
        number_format($montoOperacion, 2, ',', ''), // TXT usa coma para decimales
        number_format($porcentajeRetencion, 2, ',', '') // TXT usa coma para decimales
    ]);
