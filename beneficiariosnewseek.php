...
if ($_POST["tabla"] == "beneficiarios") {
	...
	if (!isset($_POST["borrar"])) {
		...
		$si = 0;
		if ($_POST["ModalEdoc"] == "on") {
			$a = "1";
		} else {
			$a = "0";
		}

		if ($_POST["ModalEdo"] == "on") {
			$b = "1";
		} else {
			$b = "0";
		}

		if ($_POST["ModalCli"] == "on") {
			$c = "1";
		} else {
			$c = "0";
		}

		if ($_POST["ModalProv"] == "on") {
			$d = "1";
		} else {
			$d = "0";
		}

		if ($_POST["ModalTrab"] == "on") {
			$e = "1";
		} else {
			$e = "0";
		}

		if ($_POST["ModalOtro"] == "on") {
			$f = "1";
		} else {
			$f = "0";
		}
		if ($_POST["ModalServicio"] == "on") {
			$ModalServicio = "1";
		} else {
			$ModalServicio = "0";
		}
		if ($_POST["new"] === "0") {
			...
			if ($si == 0) {
				...
				if ($IdPais == 'CL') {
					$sql = "insert into PosUpclientes (IdCompany,RUT,CodIntDeudor,codBarra,Nombre,Fono,Direccion,TipoCredito,EstadoCredito,DeudaMaxBoleta,DeudaMaxLibreta,FechPago1,FechPago2,NroCorr,Comuna,Giro,CreditoFactura,CreditoUsado,Estado,EsCliente,EsProveedor,EsTrabajador,EsOtro,email,provincia,region,ciudad,Latitud,Longitud, TipoBenef, esservidor) values(" . $_POST["companyUser"] . ",'" . trim($_POST["ModalRut"]) . "-" . strtoupper(trim($_POST["ModalRut2"])) . "','" . trim($ModalCid) . "','" . trim($ModalCodb) . "','" . trim($ModalNombre) . "','" . trim($ModalTelf) . "','" . trim($ModalDir) . "','" . trim($ModalTipoc) . "','" . $a . "',0,0,0,0,0,'" . $Comuna . "','" . trim($ModalGiro) . "',0,0,'" . $b . "','" . $c . "','" . $d . "','" . $e . "','" . $f . "','" . trim($ModalEmailCorreo) . "','" . trim($Provincia) . "','" . trim($Region) . "','" . trim($ModalCiudad) . "','" . $latitud . "','" . $longitud . "','" . $_POST["TipoBeneficiario"] . "','" . $ModalServicio . "')";
				} else {
					$sql = "insert into PosUpclientes (IdCompany,RUT,CodIntDeudor,codBarra,Nombre,Fono,Direccion,TipoCredito,EstadoCredito,DeudaMaxBoleta,DeudaMaxLibreta,FechPago1,FechPago2,NroCorr,Comuna,Giro,CreditoFactura,CreditoUsado,Estado,EsCliente,EsProveedor,EsTrabajador,EsOtro,email,provincia,region,ciudad,Latitud,Longitud, TipoBenef, esservidor) values(" . $_POST["companyUser"] . ",'" . trim($ModalRif) . "','" . trim($ModalCid) . "','" . trim($ModalCodb) . "','" . trim($ModalNombre) . "','" . trim($ModalTelf) . "','" . trim($ModalDir) . "','" . trim($ModalTipoc) . "','" . $a . "',0,0,0,0,0,'" . $Comuna . "','" . trim($ModalGiro) . "',0,0,'" . $b . "','" . $c . "','" . $d . "','" . $e . "','" . $f . "','" . trim($ModalEmailCorreo) . "','" . trim($Provincia) . "','" . trim($Region) . "','" . trim($ModalCiudad) . "','" . $latitud . "','" . $longitud . "','" . $_POST["TipoBeneficiario"] . "','" . $ModalServicio . "')";
				}
		...
		if ($_POST["new"] === "1") {
			...
			$ModalCiudad = mysqli_real_escape_string($conn, $_POST["ModalCiudad"]);
			if ($IdPais == 'CL') {
				$sql = "update PosUpclientes set TipoBenef='" . $_POST["TipoBeneficiario"] . "',Longitud='" . $longitud . "',Longitud='" . $longitud . "',Latitud='" . $longitud . "', region = '" . trim($Region) . "',provincia = '" . trim($Provincia) . "',email='" . trim($ModalEmailCorreo) . "',CodIntDeudor='" . trim($ModalCid) . "',codBarra='" . trim($ModalCodb) . "',Nombre='" . trim($ModalNombre) . "',Fono='" . trim($ModalTelf) . "',Direccion='" . trim($ModalDir) . "',TipoCredito='" . trim($ModalTipoc) . "',EstadoCredito='" . $a . "',Comuna='" . $Comuna . "',Giro='" . trim($ModalGiro) . "',Estado='" . $b . "',EsCliente='" . $c . "',EsProveedor='" . $d . "',EsTrabajador='" . $e . "',EsOtro='" . $f . "',ciudad='" . trim($ModalCiudad) . "',esservidor='" . $ModalServicio . "' where RUT='" . trim($_POST["ModalRut"]) . "-" . trim($_POST["ModalRut2"]) . "' and IdCompany=" . trim($_POST["companyUser"]);
			} else {
